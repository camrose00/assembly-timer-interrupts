...
//
	...
	.align
TIM3_IRQHandler:
 // Clear the UIF flag (bit 0) in TIM3_SR
    ldr r0, =TIM3_SR          // Load the address of TIM3_SR into r0
    ldr r1, [r0]              // Load the value of TIM3_SR into r1
    mov r2, #1                // Bit mask for bit 0
    bic r1, r1, r2            // Reset bit 0 in TIM3_SR
    str r1, [r0]              // Store the updated value back to TIM3_SR

    // Invert bit 0 in the 'task' global variable
    ldr r3, =task             // Load the address of the task variable
    ldr r4, [r3]              // Load the current value of the task variable
    eor r4, r4, #1            // Invert bit 0
    str r4, [r3]              // Store the updated value back to the task variable
